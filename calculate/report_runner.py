...
def run_report(
    ...
    #Steo50 Foreach  MyRepTempBlock in MyRepTempBlockList (YNumber2 Increasing)
    for my_rep_temp_block in my_rep_temp_block_list:
        print(f"[INFO][Step 40] Processing RepTempBlock: {my_rep_temp_block}")

        if my_rep_temp_block.now_y_block_cdt_cdt1 != 'CDT-S1-50':
            continue
        ...
        # Build initial map of field_name -> field_value for non-None fields
        field_map = {
        ...
        for field_name, field_value in field_map.items():
            ...
            if len(filter_items_df) > 0:
                to_items = filter_items_df['TO_Y_BLOCK_ToItem'].tolist()
                my_filter_item_map[field_name] = to_items
                print(f"[INFO][Step 70] {field_name} ({to_type}): Found {len(to_items)} items")
        ...
        # Generate all combinations of filter items from the map
        ...
        if my_filter_item_map:
            ...
            print(f"[INFO][Step 80] Created {len(filter_combinations)} filter combinations")
            print(f"[INFO][Step 80] First 3 combinations: {filter_combinations[:3]}")
        else:
            filter_combinations = []
            print(f"[INFO][Step 80] No filter items to combine")

        for my_filter_item in filter_combinations:
            #Step120 Foreach MyFilterItem (L incresing)
            l_items =  [0,1,2,3,4,5]
            for l in l_items:
                #Step140 MyXPeriod = MyLastReportMonth - L
                # MyLastReportMonth format: M2504 (April 2025)
                # Parse year and month from MyLastReportMonth
                year = int(my_last_report_month[1:3])  # "25" from "M2504"
                month = int(my_last_report_month[3:5])  # "04" from "M2504"
                
                # Calculate new month and year after subtracting L
                total_months = year * 12 + month - l
                new_year = total_months // 12
                new_month = total_months % 12
                
                # Handle month = 0 case (should be December of previous year)
                if new_month == 0:
                    new_month = 12
                    new_year -= 1
                
                # Format back to M{YY}{MM} format
                my_x_period = f"M{new_year:02d}{new_month:02d}"
                print(f"[INFO][Step 140] L={l}, MyLastReportMonth={my_last_report_month}, MyXPeriod={my_x_period}")

                #Step150 MyYNumber3 = L
                my_y_number_3 = l
                
                #Step160 Query from SOCell: (MyZBlockPlan, MyKRTypeFull, MyFilterItem, MyXPeriod) -> SOCell1 = NowValue //Plan
                # Parse MyZBlockPlan from my_rep_page
                ...
                # Add MyKRTypeFull conditions (map field names to SoCell field names)
                kr_field_mapping = {
                ...
                # Add MyFilterItem conditions (map field names to SoCell field names)
                ...
                if len(so_cell_df) > 0:
                    so_cell1_now_value = so_cell_df.iloc[0]['now_value']
                    print(f"[INFO][Step 160] SOCell1 NowValue (Plan): {so_cell1_now_value}")
                else:
                    so_cell1_now_value = None
                    print(f"[WARN][Step 160] No SOCell data found for Plan")

                #Step170 Query from SOCell: (Zblock = "ACT", MyKRTypeFull, MyFilterItem, MyXPeriod) -> SOCell2 = NowValue //Actual
                # Build WHERE conditions for Actual scenario
                where_conditions_actual = []
                
                # Add hardcoded Actual scenario condition
                where_conditions_actual.append("z_block_zblock1_scenario = 'Actual'")
                
                # Add MyKRTypeFull conditions
                for kr_field, kr_value in my_kr_type_full.items():
                    if kr_field in kr_field_mapping:
                        so_cell_field = kr_field_mapping[kr_field]
                        where_conditions_actual.append(f"{so_cell_field} = '{kr_value}'")
                
                # Add MyFilterItem conditions
                for filter_field, filter_value in my_filter_item.items():
                    if filter_field in filter_field_mapping:
                        so_cell_field = filter_field_mapping[filter_field]
                ...
                # Add MyXPeriod condition
                where_conditions_actual.append(f"now_np = '{my_x_period}'")
                ...
                if len(so_cell_actual_df) > 0:
                    so_cell2_now_value = so_cell_actual_df.iloc[0]['now_value']
                    print(f"[INFO][Step 170] SOCell2 NowValue (Actual): {so_cell2_now_value}")
                else:
                    so_cell2_now_value = None
                    print(f"[WARN][Step 170] No SOCell data found for Actual")

                #Step180 Query from SOCell: (MyZBlockForecast, MyKRTypeFull, MyFilterItem, MyXPeriod) -> SOCell3 = NowValue //Forecast
                # Parse MyZBlockForecast from my_rep_page
                ...
                # Build WHERE conditions for Forecast
                where_conditions_forecast = []
                
                # Add ZBlock Forecast conditions
                ...
                # Add MyXPeriod condition
                where_conditions_forecast.append(f"now_np = '{my_x_period}'")
                ...
                if len(so_cell_forecast_df) > 0:
                    so_cell3_now_value = so_cell_forecast_df.iloc[0]['now_value']
                    print(f"[INFO][Step 180] SOCell3 NowValue (Forecast): {so_cell3_now_value}")
                else:
                    so_cell3_now_value = None
                    print(f"[WARN][Step 180] No SOCell data found for Forecast")
                ...
                rep_cell_plan = RepCell(
                    ...
                    my_rep_page=my_z_block_plan,  # MyRepPage from my_z_block_plan
                ...
                last_actual_month_year = int(my_last_report_month[1:3])
                last_actual_month_month = int(my_last_report_month[3:5])
